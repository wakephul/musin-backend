...
def simulate_network(coherence, par):
    ...
    J_unit_noise = ComputePSPNorm(par['tau_m_ex'], par['C_m_ex'], par['tau_syn_noise'])
    J_norm_noise = J / J_unit_noise 

    J_unit_AMPA = ComputePSPNorm(par['tau_m_ex'], par['C_m_ex'], par['tau_syn_AMPA'])
    ...
    J_unit_GABA = ComputePSPNorm(par['tau_m_in'], par['C_m_in'], par['tau_syn_GABA'])
    ...
    rec_pop=par['rec_pop']
    rate_monitor_A, voltage_monitor_A, spike_monitor_A,  idx_monitored_neurons_A = get_monitors(pop_A, int(rec_pop*len(pop_A)))
    rate_monitor_B, voltage_monitor_B, spike_monitor_B,  idx_monitored_neurons_B = get_monitors(pop_B, int(rec_pop*len(pop_B)))
    rate_monitor_inh, voltage_monitor_inh, spike_monitor_inh,  idx_monitored_neurons_inh = get_monitors(pop_inh, int(rec_pop*len(pop_inh)))
...
def run(sim_parameters):
    ...
    coherence = 0.512
    # ret_vals, raster_A, raster_B, raster_In, activity, inputs = simulate_network(coherence,sim_parameters)
    ret_vals = simulate_network(coherence,sim_parameters)
    return ret_vals
