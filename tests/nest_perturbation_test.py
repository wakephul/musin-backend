...
# Parameters specific for the neurons in the network.
# The default values of the reset potential "E_L" and the spiking threshold "V_th"
# are used to set the limits of the initial potential of the neurons.
...
# If J is small (e.g. J = 0.1) we are likely to observe a non-chaotic network behaviour (after perturbation the network returns to its original activity). 
# Increasing J (e.g J = 5.5) leads to rather chaotic activity. Given that in this example the transition to chaos is probabilistic, 
# we sometimes observe chaotic behaviour for small weights (e.g. J = 0.5) and non-chaotic behaviour for strong weights (e.g. J = 5.4).
...
# Before we build the network we reset the simulation Kernel
# (to make sure that previous NEST simulations in the python shell will not disturb this simulation),
# and set the simulation resolution (later defined synaptic delays cannot be smaller then the simulation resolution).
nest.ResetKernel()
nest.SetStatus([0], [{"resolution": dt}])

# Now we start building the network and create excitatory and inhibitory nodes and connect them. 
# According to the connectivity specification each neurons gets KE (KI) synapses from the excitatory (inhibitory) population assigned randomly.
...
# Afterwards we create a Poisson generator that provides spikes (the external input) to the neurons until time 'T' is reached.
# Afterwards a DC generator, which is also connected to the whole population, provides a stong hyperpolarisation step for a short time period 'fade_out'. 
# In order to suppress the firing after the simulation successfully the fade out period has to last at least two times the simulation resolution.
...
for trial in [0, 1]:
    # As mentioned above, we need to reset the network, the random number generator, and the clock of the simulation Kernel. 
    # In addition we make sure that there is no spike left in the spike detector.
    nest.ResetNetwork()
    ...
    # In the second trial we add an extra input spike at time t_stim to the neuron that fires first after perturbation time t_stim. 
    # Thus, we make sure that the perturbation is transmitted to the network before it fades away in the perturbed neuron. 
    # (Single IAF-neurons are not chaotic.)
    if trial == 1:
